...
export async function createEditor(
  ...
  const start = await addNode(100, 2000);
  const end = await addNode(5400, 2400);

  for (let i = 0; i < numberOfNodes - 2; i++) {
    const { x, y } = randomPosition();
    const a = await addNode(x, y);

    await editor.addConnection(new Connection(start, "key", a, "key"));
    await editor.addConnection(new Connection(a, "key", end, "key"));
  }
